    ...
    try {
      ...
      setMessage(
        res.data.overallRating
          ? `${res.data.name} added — rated ${res.data.overallRating}/10 (${res.data.pool}), base price ${formatPrice(res.data.basePrice)}`
    ...
    <div className="w-full bg-[#0f1729] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl shadow-black/40">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs text-slate-400 uppercase tracking-wider mb-1.5 block sm:visible">Link User</label>
            <select
              value={userId}
        ...
        <div>
          ...
          <select
            ...
            value={formData.role}
            onChange={handleChange}
            className="w-full sm:w-1/2 px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white outline-none focus:border-[#f4b942] focus:ring-1 focus:ring-[#f4b942] transition-colors text-sm"
          >
        ...
        <button
          type="submit"
          disabled={saving}
          className="mt-2 w-full sm:w-auto sm:self-end bg-[#f4b942] hover:bg-[#e5aa2f] disabled:opacity-50 disabled:cursor-not-allowed text-[#0a0f1e] font-display font-semibold text-[15px] tracking-wide px-6 py-3 rounded-lg transition-colors"
        >
