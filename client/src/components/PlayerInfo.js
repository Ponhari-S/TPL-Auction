...
const PlayerInfo = () => {
    ...
    if (loading) {
        return (
            <div className="w-full max-w-3xl mx-auto bg-[#0f1729] border border-white/10 rounded-2xl p-6 flex items-center justify-center gap-3">
    ...
    if (error) {
        return (
            <div className="w-full max-w-3xl mx-auto bg-[#0f1729] border border-red-500/30 rounded-2xl p-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div>
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                      <span className="px-3 py-1 rounded-full bg-[#f4b942]/10 border border-[#f4b942]/30 text-[#f4b942] text-xs uppercase tracking-wider capitalize">
                          {player.role}
                      </span>
                      {player.overallRating && (
                          <span className="bg-[#f4b942]/10 border border-[#f4b942]/30 text-[#f4b942] text-xs font-display px-3 py-1 rounded-full">
              ...
              <span className={`self-start px-3 py-1 rounded-full border text-xs uppercase tracking-wider capitalize whitespace-nowrap ${statusStyles[player.status] || "bg-white/5 text-slate-300 border-white/10"}`}>
                  {player.status}
              </span>
          </div>
          ...
          {user?.role !== 'captain' && player.status !== 'sold' && player.status !== 'registered' && (
              <div className="mt-6 pt-5 border-t border-white/10 flex items-center justify-between gap-4 flex-wrap">
                  ...
                  <button
                      ...
                      disabled={registered}
                      className="w-full sm:w-auto bg-[#f4b942] hover:bg-[#e5aa2f] disabled:opacity-50 disabled:cursor-not-allowed text-[#0a0f1e] font-display font-semibold text-sm px-5 py-2.5 rounded-lg transition-colors"
                  >
