...
const AuctionRules = () => {
    ...
    return (
        <div className="min-h-screen lg:h-screen lg:overflow-y-auto bg-slate-900 flex items-center justify-center p-4 sm:p-6 lg:p-8">
          <div className="w-full max-w-md lg:max-w-3xl xl:max-w-4xl bg-[#0f1729] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl shadow-black/40">
            <style>{`
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {rules.minBidIncrementRules?.map((tier, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <span className="text-slate-500 text-xs w-24">
                        Up to ₹{tier.upTo === null ? '∞' : tier.upTo.toLocaleString()}
                      </span>
                      <input
                        type="number"
                        value={tier.increment}
                        onChange={(e) => {
                          const updated = [...rules.minBidIncrementRules];
              ...
              <button
                type="submit"
                className="mt-2 lg:w-1/3 lg:self-end bg-[#f4b942] hover:bg-[#e5aa2f] text-[#0a0f1e] font-display font-semibold text-[15px] tracking-wide py-3 rounded-lg transition-colors"
              >
                Save Rules
              </button>
            </form>
