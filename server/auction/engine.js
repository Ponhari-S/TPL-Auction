...
const placeBid = async (userId, amount) => {
  ...
  const playerDoc = await Player.findById(state.currentPlayer).select('name');
  console.log(`Bid placed: ${team.name} bid ${amount} on player ${playerDoc?.name || state.currentPlayer}`);
  return { success: true };
};
